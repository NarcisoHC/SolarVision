...
def segment(upload):
    ...
    bucket = storage_client.bucket(bucket_name)
    blob = bucket.blob(source_blob_name)
    blob.download_to_filename("image_to_segment.png")
    
    image = Image.open("image_to_segment.png").convert('RGB')
    ...
    test.save("segmented_image.png")
    
    source_blob_name_2 = "data/segment_image/segmented_image.png"
    blob = bucket.blob(source_blob_name_2)
    blob.upload_from_filename("segmented_image.png")

    return dict(mask = "https://storage.cloud.google.com/solarvision-test/data/segment_image/segmented_image.png")
